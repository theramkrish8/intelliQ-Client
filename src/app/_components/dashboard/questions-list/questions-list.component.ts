...
export class QuestionsListComponent implements OnInit, OnDestroy {
	...
	initQuestions(user: User) {
		if (
			this.router.url === '/dashboard/teacher/my-questions' ||
			this.router.url === '/dashboard/approver/my-questions'
		) {
	...
	onFilterApplied() {
		if (this.selectedGrade !== '-1' && this.selectedSubject !== 'default') {
			this.questions = this.allQuestions.filter(
				(t) => t.std === Number(this.selectedGrade) && t.subject === this.selectedSubject
			);
